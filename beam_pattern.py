...
def plot_beam_pattern(theta_bins, w_fft_dB, theta_soi_deg, nulls_deg):
    ...
    ax.plot(
        [theta_soi_deg / 360 * np.pi], [np.max(w_fft_dB)], "og", label="Main beam"
    )  # Zielona kropka dla głównej wiązki
...
theta_bins_rotated, w_fft_dB_rotated = rotate_beam_pattern(
    theta_bins, w_fft_dB, theta_soi_deg
)

# STA2 odbiera moc i kąt
sta2_angle_deg = 270  # Kąt STA2 względem anteny
sta2_power = calculate_power_at_angle(
    theta_bins_rotated, w_fft_dB_rotated, sta2_angle_deg
)
...
def check_beam_symmetry(theta_bins, w_fft_dB):
    ...
    for i in range(half_len):
        ...
        print(
            f"Kąt: {angles_deg_sorted[i]:.1f}° vs {angles_deg_sorted[-(i + 1)]:.1f}° -> Zysk: {gain_left:.2f} dB vs {gain_right:.2f} dB"
        )
