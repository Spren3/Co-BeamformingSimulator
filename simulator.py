...
class TopologyGenerator:
    ...
    def generate_multiroom_topology(
        self,
        topo_seed: int,
        grid_size: Tuple[int, int],
        room_size: float,
        stations_per_room: int = 4,
    ) -> Dict:
        """
    ...
    def generate_open_space_topology(
        self,
        topo_seed: int,
        area_size: float = 75.0,
        num_aps: int = 4,
        stations_per_ap: Tuple[int, int] = (3, 4),
        station_std: float = 10.0,
    ) -> Dict:
        """
    ...
    def calculate_interference_omni(
        self,
        ...
        for interfering_ap_id, interfering_sta_id in all_transmissions:
            if interfering_ap_id != target_ap_id:  # Pomijamy AP docelowy
                ...
                print(
                    f"Interferujący AP {interfering_ap_id} na pozycji ({interfering_ap_node.x:.2f}, {interfering_ap_node.y:.2f})"
                )
                print(
                    f"Interferująca STA {interfering_sta_id} na pozycji ({interfering_sta_node.x:.2f}, {interfering_sta_node.y:.2f})"
                )
    ...
    def calculate_interference_with_antennas(
        self,
        target_link: Tuple[int, int],
        all_transmissions: List[Tuple[int, int]],
        nodes: List[NetworkNode],
        ...
        for interfering_ap_id, interfering_sta_id in all_transmissions:
            if interfering_ap_id != target_ap_id:  # Pomijamy AP docelowy
                ...
                tx_angle = self.calculate_angle(
                    interfering_ap_node, interfering_sta_node
                )
                theta_bins_rotated, w_fft_dB_rotated = rotate_beam_pattern(
                    theta_bins, w_fft_dB, tx_angle
                )
                tx_antenna_gain_db = calculate_power_at_angle(
                    theta_bins_rotated, w_fft_dB_rotated, tx_angle
                )
                ...
                interference_angle = self.calculate_angle(
                    interfering_ap_node, target_sta_node
                )
                interference_antenna_gain_db = calculate_power_at_angle(
                    theta_bins_rotated, w_fft_dB_rotated, interference_angle
                )
        ...
        for node in nodes:
            if node.node_type == "AP":
                ax.scatter(
                    ...
                    marker="x",
                    label="AP"
                    if node.id == min(n.id for n in nodes if n.node_type == "AP")
                    else "",
                    zorder=5,
                )
            else:
                ax.scatter(
                    ...
                    marker="o",
                    label="STA"
                    if node.id == min(n.id for n in nodes if n.node_type == "STA")
                    else "",
                    zorder=5,
                )
...
def round_sim(
    ...
    topology = generator.generate_multiroom_topology(
        topo_seed=topology_seed, grid_size=(2, 2), room_size=20.0
    )
    ...
    for sim in range(num_simulations):
        ...
        if ap_selection == "pojedyncze":
            ...
            selected_aps = np.random.choice(
                aps, size=np.random.randint(1, len(aps) + 1), replace=False
            )
            ...
            for ap in aps:
                sta_distances = [
                    (sta, np.sqrt((ap.x - sta.x) ** 2 + (ap.y - sta.y) ** 2))
                    for sta in stations
                ]
            ...
            selected_aps = (
                [ap_sta_distances[0][0], ap_sta_distances[1][0], ap_sta_distances[2][0]]
                if len(ap_sta_distances) > 1
                else [ap_sta_distances[0][0]]
            )
        else:
        ...
        transmissions = [
            (
                ap.id,
                random.choice(
                    [sta.id for sta in stations if sta.associated_ap == ap.id]
                ),
            )
        ...
        for link in transmissions:
            ...
            if pattern_type == "beam":
                ...
                theta_bins_rot, w_fft_dB_rot = rotate_beam_pattern(
            ...
            total_thr += round_thr
            print(
                f"Kąt do STA: {angle:.2f}°, odległość: {d}, zysk anteny: {gain:.2f} dB, path loss: {pl:.2f} dB, interf: {10 * np.log10(interference):.2f} dBm, SINR: {sinr:.2f} dB, przepustowość: {thr} Mbps"
            )
        print(
            "-----Całkowita przepustowość po ", sim, "rundzie to :", round_thr, "-----"
        )
