...
from beam_pattern import calculate_beam_pattern, calculate_power_at_angle, rotate_beam_pattern
    ...
    def interf(self, STA, AP, all_active_transmissions=None):
        ...
        if all_active_transmissions is None:
            ...
            for tx_info in all_active_transmissions:
                ...
                angle_interferer = angle_between(STA, tx_ap)[1]
...
def fourth_scenario_2AP_2STA(d):
    ...
    for i in range(1,d):
        sta4[1]=sta4[1]-1
        sta5[1]=sta5[1]+1
        angle1=angle_between(sta4,ap3)[1]
        angle1int=angle_between(sta4,ap4)[1]
        angle2=angle_between(sta5,ap4)[1]
        angle2int=angle_between(sta5,ap3)[1]
        ...
        print("kat miedzy sta4, a AP3: ",angle2, "moc kierowana to: ", antenna_gain2, "pozycja sta to: ",sta5)
        sinr_omni[i] = Tx_PWR - (scen4.path_loss(d,f)+10*np.log10(noise))
        rate_omni = sinr_to_mcs(sinr_omni[i])[1]
        throughput_omni[i] = rate_omni
        
        ##SINR z antenna gain
        sinr_beam[i] = scen4.SINR(sta5,ap4, antenna_gain2,all_active_transmissions)
    ...
    ax1.set_xlabel("Odległość między STA")
    ax1.set_ylabel("SINR (dB)")
