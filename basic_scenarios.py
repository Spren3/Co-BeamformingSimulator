...
stas = np.array(
    [sta1, sta2]
)  # zmienne zeby zgrupowac stacje i AP do wyswietlania na wykresie
objects = np.array([ap1, ap2, sta1, sta2])
theta_bins, w_fft_dB = calculate_beam_pattern(
    8, 0.5, 0 / 360 * np.pi, np.asarray(np.linspace(-60, 60, 11)) / 360 * np.pi
)
...
class calculations:
    ...
    def interf(self, STA, AP, all_active_transmissions=None):
        ...
        if all_active_transmissions is None:
            ...
            for interfering_AP in self.aps:
                ...
                interference_mW = pow(
                    10, (Tx_PWR - self.path_loss(d_interferer, f)) / 10
                )
            ...
            for tx_info in all_active_transmissions:
                ...
                angle_interferer = angle_between_points_from_perspective(
                    tx_ap, STA, rx_sta
                )
                print(
                    "pozycje sta i ap int: ",
                    STA,
                    tx_ap,
                    "kąt miedzy STA, a int: ",
                    angle_interferer,
                )
                ...
                interference_gain = calculate_power_at_angle(
                    theta_bins, w_fft_dB, angle_interferer
                )
    ...
    def SINR(self, STA, AP, antenna_gain=0, all_active_transmissions=None):
        ...
        print(
            "Kontrolnie wypisanie wszystkiego: ",
            clAP,
            d2,
            self.path_loss(d, f),
            10 * log10(self.interf(STA, AP, all_active_transmissions)),
        )  ##zakomentowane
        sinr = (Tx_PWR + antenna_gain) - (
            self.path_loss(d, f)
            + 10 * np.log10(self.interf(STA, AP, all_active_transmissions) + noise)
        )
        return sinr
    ...
    def SINR_beamforming(
        ...
        for ap_int, angle_int in zip(interfering_APs, interfering_angles):
            interference += pow(
                10,
                self.SSB(
                    ap_int, angle_int, STA, theta_bins=theta_bins, w_fft_dB=w_fft_dB
                )
                / 10,
            )
...
def plot_means_with_ci(
    ...
    plt.bar(
        x,
        means,
        yerr=[lo_bounds, hi_bounds],
        capsize=capsize,
        color=[colors[(n_vars - 1 - i) % len(colors)] for i in range(n_vars)],
    )
    plt.xticks(x, labels)
...
def plot_histograms(
    transmission_pairs, receiver_sets, pattern_type, ap_selection, seed, num_simulations
):
    ...
    filtered_pairs = [
        ...
        if (count / total_transmissions * 100) > threshold
    ]
    if filtered_pairs:
        labels = [f"({tx}, {rx})" for (tx, rx), _ in filtered_pairs]
        frequencies = [
            (count / total_transmissions * 100) for _, count in filtered_pairs
        ]
        ...
        plt.tight_layout()
        plt.savefig(
            f"histograms/histogram_pary_{pattern_type}_{ap_selection}_{seed}_{num_simulations}.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close("all")
    ...
    filtered_sets = [
        (rx_set, count)
        for rx_set, count in sorted_sets
        if (count / total_sets * 100) > threshold
    ]
    ...
    if filtered_sets:
        ...
        plt.tight_layout()
        plt.savefig(
            f"histograms/histogram_zbiory_rx_{pattern_type}_{ap_selection}_{seed}_{num_simulations}.pdf",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close("all")
...
def first_scenario_2AP_STA_moving(AP1, AP2, distance):
    ...
    for i in range(AP1[0] + 10, distance, 1):
        ...
        theta_bins_rotated, w_fft_dB_rotated = rotate_beam_pattern(
            theta_bins, w_fft_dB, angle
        )
        antenna_gain = calculate_power_at_angle(
            theta_bins_rotated, w_fft_dB_rotated, angle
        )
        ...
        sinr_without_interference[i] = (
            Tx_PWR
            + antenna_gain
            - (firstscenario.path_loss(d, f) + 10 * np.log10(noise))
        )
    ...
    ax1.plot(
        sinr_with_interference.keys(),
        sinr_with_interference.values(),
        label="SINR with interference",
        color="b",
        marker="o",
        linestyle="-",
    )
    ax1.plot(
        sinr_without_interference.keys(),
        sinr_without_interference.values(),
        label="SINR without interference",
        color="g",
        marker="x",
        linestyle="--",
    )
    ...
    ax2.plot(
        throughput_with_interference.keys(),
        throughput_with_interference.values(),
        label="Throughput with interference",
        color="r",
        linestyle="-.",
    )
    ax2.plot(
        throughput_without_interference.keys(),
        throughput_without_interference.values(),
        label="Throughput without interference",
        color="orange",
        linestyle=":",
    )
...
def second_scenario_AP_moving(distance):
    ...
    for i in range(sta[0] + 20, sta[0] + distance, 1):
        ...
        sinr_without_interference[i] = (
            Tx_PWR
            + antenna_gain
            - (scecond_scen.path_loss(d, f) + 10 * np.log10(noise))
        )
        rate_without_interference = sinr_to_mcs(sinr_without_interference[i])[1]
        throughput_without_interference[i] = rate_without_interference
    ...
    ax2.plot(
        throughput_with_interference.keys(),
        throughput_with_interference.values(),
        label="Przepustowość z interferencją",
        color="r",
        linestyle="-.",
    )
    ax2.plot(
        throughput_without_interference.keys(),
        throughput_without_interference.values(),
        label="Przepustowość bez interferencji",
        color="orange",
        linestyle=":",
    )
...
def fourth_scenario_2AP_2STA(d):
    ...
    throughput_beam = {}
    throughput_beam2 = {}
    theta_bins1, w_fft_dB1 = calculate_beam_pattern(
        8, 0.5, 0, np.asarray(np.linspace(-60, 60, 11)) / 360 * np.pi
    )
    theta_bins2, w_fft_dB2 = calculate_beam_pattern(
        8, 0.5, 0, np.asarray(np.linspace(-60, 60, 11)) / 360 * np.pi
    )
    plt.figure(figsize=(8, 8))
    plt.scatter(aps[:, 0], aps[:, 1], c="red", label="AP", marker="x", s=100)
    ...
    for i in range(1, d):
        ...
        # SINR
        theta_bins1_rotated, w_fft_dB1_rotated = rotate_beam_pattern(
            theta_bins1, w_fft_dB1, angle1
        )
        theta_bins2_rotated, w_fft_dB2_rotated = rotate_beam_pattern(
            theta_bins2, w_fft_dB2, angle2
        )

        # Obliczanie wzmocnienia dla docelowych stacji
        antenna_gain1 = calculate_power_at_angle(
            theta_bins1_rotated, w_fft_dB1_rotated, angle1
        )
        antenna_gain2 = calculate_power_at_angle(
            theta_bins2_rotated, w_fft_dB2_rotated, angle2
        )
    ...
    ax2 = ax1.twinx()  # Druga oś Y
    ax2.plot(
        throughput_omni.keys(),
        throughput_omni.values(),
        label="Przepustowość omni",
        color="r",
        linestyle="-.",
    )
    ax2.plot(
        throughput_beam.keys(),
        throughput_beam.values(),
        label="Przepustowość beam",
        color="orange",
        linestyle=":",
    )
...
def scenario_2AP_2STA_beamforming(d):
    ...
    alpha1, beta1, gamma1, delta1 = get_beam_angles_and_delta(
        ap3, sta4_fixed, ap4, sta5_fixed
    )
    print(
        f"Dla stałych miejsc Alpha1: {alpha1}, Beta1: {beta1}, Gamma1: {gamma1}, Delta1: {delta1}"
    )
    ...
    all_active_transmissions = [
        {"ap": ap3, "sta": sta4, "beam_pattern": (theta_bins1, w_fft_dB1)},
        {"ap": ap4, "sta": sta5, "beam_pattern": (theta_bins2, w_fft_dB2)},
    ]

    # 2. Oba AP nadają (omni)
    interf_ap3 = pow(
        10, (Tx_PWR - scen2.path_loss(np.linalg.norm(sta4_fixed - ap4), f)) / 10
    )
    interf_ap4 = pow(
        10, (Tx_PWR - scen2.path_loss(np.linalg.norm(sta5_fixed - ap3), f)) / 10
    )
    sinr_omni_both_ap3 = Tx_PWR - (
        scen2.path_loss(d_ap3_sta4, f) + 10 * np.log10(interf_ap3 + noise)
    )
    sinr_omni_both_ap4 = Tx_PWR - (
        scen2.path_loss(d_ap4_sta5, f) + 10 * np.log10(interf_ap4 + noise)
    )
    ...
    print(
        "kąty dla tylko 1 AP nadającego: ap3 ", angle_ap3_sta4, "ap4: ", angle_ap4_sta5
    )
    theta_bins1_rot, w_fft_dB1_rot = rotate_beam_pattern(
        theta_bins1, w_fft_dB1, angle_ap3_sta4
    )
    theta_bins2_rot, w_fft_dB2_rot = rotate_beam_pattern(
        theta_bins2, w_fft_dB2, angle_ap4_sta5
    )
    ...
    sinr_beam_ap3 = (
        Tx_PWR + gain_ap3 - (scen2.path_loss(d_ap3_sta4, f) + 10 * np.log10(noise))
    )
    sinr_beam_ap4 = (
        Tx_PWR + gain_ap4 - (scen2.path_loss(d_ap4_sta5, f) + 10 * np.log10(noise))
    )
    ...
    gain_ap4_to_sta4 = calculate_power_at_angle(
        theta_bins2_rot, w_fft_dB2_rot, angle_ap4_sta4
    )
    interf_beam_ap3 = pow(
        10,
        (
            Tx_PWR
            + gain_ap4_to_sta4
            - scen.path_loss(np.linalg.norm(sta4_fixed - ap4), f)
        )
        / 10,
    )
    sinr_beam_both_ap3 = scen.SSB(
        ap3, angle_ap3_sta4, sta4_fixed, theta_bins=theta_bins1, w_fft_dB=w_fft_dB1
    ) - 10 * np.log10(interf_beam_ap3 + noise)
    thr_beam_both_ap3 = sinr_to_mcs(sinr_beam_both_ap3)[1]

    angle_ap3_sta5 = angle_between(ap3, sta5_fixed)
    gain_ap3_to_sta4 = calculate_power_at_angle(
        theta_bins1_rot, w_fft_dB1_rot, angle_ap3_sta4
    )
    interf_beam_ap4 = pow(
        10,
        (
    ...
    sinr_beam_both_ap4 = scen.SSB(
        ap4, angle_ap4_sta5, sta5_fixed, theta_bins=theta_bins2, w_fft_dB=w_fft_dB2
    ) - 10 * np.log10(interf_beam_ap4 + noise)
    print(
        "wszystkie sily wchodzące dla ap4: ",
        Tx_PWR,
        gain_ap3_to_sta4,
        "scen.path_loss: ",
        scen2.path_loss(np.linalg.norm(sta4_fixed - ap3), f),
        "interf_beam_ap4: ",
        np.log10(interf_beam_ap4),
        "noise: ",
        noise,
    )
    thr_beam_both_ap4 = sinr_to_mcs(sinr_beam_both_ap4)[1]

    labels = ["1 AP omni", "2 AP omni", "1 AP beam", "2 AP beam"]
    sinr_sta4 = [sinr_omni_ap3, sinr_omni_both_ap3, sinr_beam_ap3, sinr_beam_both_ap3]
    sinr_sta5 = [sinr_omni_ap4, sinr_omni_both_ap4, sinr_beam_ap4, sinr_beam_both_ap4]
    thr_sta4 = [thr_omni_ap3, thr_omni_both_ap3, thr_beam_ap3, thr_beam_both_ap3]
    thr_sta5 = [thr_omni_ap4, thr_omni_both_ap4, thr_beam_ap4, thr_beam_both_ap4]
    ...
    plt.figure(figsize=(8, 8))
    plt.scatter(
        [ap3[0], ap4[0]], [ap3[1], ap4[1]], c="red", label="AP", marker="x", s=100
    )
    plt.scatter(
        [sta4_fixed[0], sta5_fixed[0]],
        [sta4_fixed[1], sta5_fixed[1]],
        c="blue",
        label="STA",
        marker="o",
        s=100,
...
def fifth_scenario_4STA_2AP_line(d1=10, d2_range=None):
    ...
    for d2 in d2_range:
        ...
        def get_sinr_thr(AP, STA, AP_int, STA_int, use_beam, theta_bins, w_fft_dB):
            if use_beam:
                ...
                d = np.linalg.norm(STA - AP)
                interf = pow(
                    10, (Tx_PWR - scen.path_loss(np.linalg.norm(STA - AP_int), f)) / 10
                )
    ...
    aggregate_throughput = [
        np.sum(results["omni_outer"]),
        np.sum(results["beam_outer"]),
        np.sum(results["omni_inner"]),
        np.sum(results["beam_inner"]),
        np.sum(results["omni_left"]),
        np.sum(results["beam_left"]),
        np.sum(results["omni_right"]),
        np.sum(results["beam_right"]),
    ]
    aggregate_labels = [
        "Omni outer",
        "Beam outer",
    ...
    plt.bar(
        aggregate_labels, aggregate_throughput, color=["b", "g", "c", "r", "orange"]
    )
    plt.ylabel("Aggregate throughput [Mb/s]")
    plt.grid(axis="y", linestyle="--", alpha=0.7)
    plt.tight_layout()
